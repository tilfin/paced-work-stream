...
const WrapFuncPromiseAll = require('./wrap_func_promise_all');
 ...
 * @param {Boolean} opts.delay - start concurrent process in order delay for a time that divided workMS by concurrency, default is false. workPromise must return functions wrap each promise.
...
function PacedWorkStream(opts, workPromise) {
  ...
  this._delay = opts_.delay || false;
  delete opts_.concurrency;
  delete opts_.workMS;
  delete opts_.wrapFunc;
...
PacedWorkStream.prototype._process = function(promises, cb) {
  ...
  (() => {
    if (this._delay) {
      return new WrapFuncPromiseAll(promises)
        .execute(Math.floor(this._workMS / this._concurrency));
    } else {
